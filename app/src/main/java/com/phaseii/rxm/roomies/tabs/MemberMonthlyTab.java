...
import com.phaseii.rxm.roomies.view.CustomMarkerView;
...
public class MemberMonthlyTab extends RoomiesFragment {
    ...
    public View onCreateView(LayoutInflater inflater, ViewGroup container, Bundle savedInstanceState) {
        rootView = inflater.inflate(R.layout.membermonthly, container, false);
        LineChart lineChart = (LineChart) rootView.findViewById(R.id.member_monthly);
    ...
    private LineData getLineChart(List<RoomExpenses> expenses) {
        ...
        int totalWeeks = cal.getActualMaximum(Calendar.WEEK_OF_MONTH);

        for (int i = 1; i <= totalWeeks; i++) {
            float val = 0f;
            for (RoomExpenses memberExpense : memberExpenses) {
                cal.setTime(memberExpense.getExpenseDate());
                if (i == cal.get(Calendar.WEEK_OF_MONTH)) {
